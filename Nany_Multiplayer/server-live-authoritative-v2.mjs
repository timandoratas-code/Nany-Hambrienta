...
function makeFish(w,index,r=Math.random){
  const t=typeAt(index), a=r()*Math.PI*2, s=t.speed*(.82+r()*.30);
  const id=`${w.code}-fish-${index}-${w.nextFish++}`;
...
function teamFor(w,want){
  ...
  const a=[...w.players.values()].filter(p=>p.connected&&p.team==='A').length;
  const b=[...w.players.values()].filter(p=>p.connected&&p.team==='B').length;
...
function pubPlayer(p){
  return {id:p.id,name:p.name,deviceId:p.deviceId,x:+p.x.toFixed(2),y:+p.y.toFixed(2),vx:+p.vx.toFixed(2),vy:+p.vy.toFixed(2),
    angle:+p.angle.toFixed(3),score:Math.floor(p.score),growthScore:Math.floor(p.score),radius:+radius(p.score).toFixed(2),
    power:power(p.score),level:p.level,lives:p.lives,alive:p.alive,sprinting:p.sprinting,team:p.team||null,
    teamName:p.team==='A'?'Azul':p.team==='B'?'Rojo':null};
}
function pubFish(f){
  return {id:f.id,sharedId:f.id,type:f.type,role:f.role,power:f.power,points:f.points,size:+f.size.toFixed(2),color:f.color,
    x:+f.x.toFixed(2),y:+f.y.toFixed(2),vx:+f.vx.toFixed(4),vy:+f.vy.toFixed(4),angle:+f.angle.toFixed(4)};
}
function snapshot(w){
  return {type:'snapshot',room:w.code,mode:w.mode,population:[...w.players.values()].filter(p=>p.connected).length,
    serverTick:w.tick,serverTime:Date.now(),worldSeed:w.seed,worldEpoch:w.epoch,worldStage:w.stage,boss:null,removed:[],
    players:[...w.players.values()].filter(p=>p.connected).map(pubPlayer),entities:[...w.fish.values()].map(pubFish)};
}
function broadcast(w){
  const base=snapshot(w);
  for(const p of w.players.values()) if(p.connected) send(p.ws,{...base,you:pubPlayer(p),player:pubPlayer(p)});
}
function broadcastStage(w){
  sendAll(w,{type:'world_stage',stage:w.stage,serverTime:Date.now(),worldSeed:w.seed,worldEpoch:w.epoch});
  broadcast(w);
}
...
function updateFish(w,dt){
  ...
  const now=Date.now();
  const players=[...w.players.values()].filter(p=>p.connected&&p.alive);
  for(let i=w.respawns.length-1;i>=0;i--){
    const rr=w.respawns[i];
    if(now<rr.at) continue;
    const f=makeFish(w,rr.index);
    w.fish.set(f.id,f);
  ...
  for(const f of w.fish.values()){
    ...
    if(!target&&f.cooldownUntil<=now){
      ...
      for(const p of players){
        const dd=dist(f,p),pp=power(p.score),pr=radius(p.score);
        const can=f.role==='predator'?f.power>pp&&f.size>pr*1.02&&dd<340:pp>f.power&&pr>f.size*.98&&dd<180;
    ...
    if(target){
      const a=Math.atan2(target.y-f.y,target.x-f.x),k=f.role==='predator'?.24:.18;
...
function killPlayer(w,p,reason,eater=null){
  ...
  broadcast(w);
  setTimeout(()=>{
    const q=w.players.get(p.id); if(!q) return;
    const pos=spawn(w);
    q.x=pos.x;q.y=pos.y;q.tx=pos.x;q.ty=pos.y;q.lastX=pos.x;q.lastY=pos.y;q.vx=0;q.vy=0;
    q.score=Math.max(0,Math.floor(q.score*.5));q.radius=radius(q.score);q.alive=true;q.lives=Math.max(1,q.lives);
    q.invulnerableUntil=Date.now()+1500;
    send(q.ws,{type:'respawn',player:pubPlayer(q)});
    broadcast(w);
  },1000);
}
function removeFish(w,p,f){
  ...
  w.fish.delete(f.id);
  p.score+=f.points;
  p.radius=radius(p.score);
  w.respawns.push({index:f.index,at:Date.now()+RESPAWN_MS});
  sendAll(w,{type:'entity_removed',entityId:f.id,by:p.id,serverTick:w.tick,serverTime:Date.now()});
  return true;
}
function tryConsume(w,p,id){
  const f=w.fish.get(String(id||''));
  if(!f||!p.connected||!p.alive) return false;
  const pp=power(p.score),pr=radius(p.score);
  if(dist(p,f)>pr*.95+f.size*.95) return false;
  if(!(f.power<pp&&f.size<pr)) return false;
  const ok=removeFish(w,p,f);
  if(ok){maybeAdvanceStage(w);broadcast(w);}
  return ok;
}
function combat(w){
  if(w.stage===1||w.stage===3) return;
  ...
  for(const p of w.players.values()){
    ...
    for(const f of [...w.fish.values()]){
      const dd=dist(p,f);
      if(dd>pr*.86+f.size*.86) continue;
      if(f.power<pp&&f.size<pr){ removeFish(w,p,f); continue; }
      if(f.power>pp&&f.size>pr*1.02){
        p.lives=Math.max(0,p.lives-1);p.score=Math.max(0,Math.floor(p.score*.5));p.radius=radius(p.score);
        p.invulnerableUntil=now+2500;
        if(p.lives<=0) killPlayer(w,p,'fish');
        break;
      }
    }
  }
  ...
  for(let i=0;i<ps.length;i++) for(let j=i+1;j<ps.length;j++){
    ...
    const dd=dist(a,b),ar=radius(a.score),br=radius(b.score);
    if(dd>ar*.82+br*.82) continue;
    if(a.score>=b.score+1&&ar>br){
      a.score+=Math.max(10,Math.floor(b.score*.25));a.radius=radius(a.score);killPlayer(w,b,'player',a);
    }else if(b.score>=a.score+1&&br>ar){
      b.score+=Math.max(10,Math.floor(a.score*.25));b.radius=radius(b.score);killPlayer(w,a,'player',b);
    }
  }
  maybeAdvanceStage(w);
}
function movePlayers(w){
  for(const p of w.players.values()){
    if(!p.connected||!p.alive) continue;
    const dx=p.tx-p.x,dy=p.ty-p.y,dd=Math.hypot(dx,dy);
    if(dd>.01){
      const s=Math.min(dd,14);
      p.vx=dx/dd*s;p.vy=dy/dd*s;p.x=clamp(p.x+p.vx,0,W);p.y=clamp(p.y+p.vy,0,H);p.angle=Math.atan2(p.vy,p.vx);
    }else{p.vx*=.75;p.vy*=.75;}
...
function applyInput(p,m){
  ...
  const dx=x-p.lastX,dy=y-p.lastY,dd=Math.hypot(dx,dy);
  p.lastX=x;p.lastY=y;
  if(dd>.05){
    const s=Math.min(dd,12);
    p.tx=clamp(p.x+dx/dd*s,0,W);p.ty=clamp(p.y+dy/dd*s,0,H);
  }
...
const device=()=>{let d=localStorage.getItem(DEVICE);if(!d){d=(${makeDeviceId.toString()})();localStorage.setItem(DEVICE,d)}return d};
window.__NANY_DEVICE_ID__=device();
function WrappedWS(url,protocols){
  let real=null,stopped=false,retry=0,join=null,lastState=null,api;
  const ev={open:new Set(),message:new Set(),close:new Set(),error:new Set()};
  const emit=(t,e)=>{for(const f of ev[t]||[])try{f.call(api,e)}catch{}const h=api['on'+t];if(typeof h==='function')try{h.call(api,e)}catch{}};
  api={};Object.setPrototypeOf(api,Native.prototype);
  Object.defineProperty(api,'readyState',{get:()=>stopped?3:(real?.readyState??0)});
  Object.defineProperty(api,'url',{value:url});
  ...
  function connect(){
    if(stopped)return;
    real=protocols===undefined?new Native(url):new Native(url,protocols);
    ...
    real.addEventListener('message',e=>{try{const m=JSON.parse(e.data);if(m.type==='welcome')localStorage.setItem('__NANY_RESUME_ID__',m.resumeId||m.id||'');if(m.type==='snapshot'||m.type==='welcome'){const b=window[BUF]||(window[BUF]=[]);b.push({t:Number(m.serverTime)||Date.now(),m});while(b.length>40)b.shift()}}catch{}emit('message',e)});
    real.addEventListener('close',e=>{if(stopped){emit('close',e);return}setTimeout(connect,Math.min(5000,250*Math.pow(1.5,retry++)))});
    real.addEventListener('error',e=>{if(stopped)emit('error',e)});
  }
  connect();return api;
}
for(const k of ['CONNECTING','OPEN','CLOSING','CLOSED'])WrappedWS[k]=Native[k];WrappedWS.prototype=Native.prototype;window.WebSocket=WrappedWS;
...
window.__SERVER_SYNC_FISH__=function(){
  try{
    ...
    const renderAt=Date.now()-90;let B=b[b.length-1],A=B;
    for(let i=b.length-1;i>=0;i--){if(b[i].t<=renderAt){A=b[i];break}}
    const bm=B.m,am=A.m,me=bm.you||bm.player||am.you||am.player;
    if(me&&!me.alive){g.entities=[];return}
    const span=Math.max(1,(Number(bm.serverTime)||B.t)-(Number(am.serverTime)||A.t));
    const alpha=A===B?1:Math.max(0,Math.min(1,(renderAt-(Number(am.serverTime)||A.t))/span));
    const old=new Map((am.entities||[]).map(f=>[f.id,f])),out=[];
    for(const fb of bm.entities||[]){
      const fa=old.get(fb.id)||fb;
      const sp=Math.max(.1,Math.hypot(Number(fb.vx)||0,Number(fb.vy)||0));
      out.push({sharedId:fb.id,serverId:fb.id,type:fb.type,family:fb.type,variant:fb.role==='predator'?'big':'small',renderKey:fb.type,
        power:fb.power,tier:fb.power||0,ecologyRole:fb.role,size:fb.size,baseSize:fb.size,points:fb.points,color:fb.color,
        behavior:fb.role==='predator'?'aggro':'drift',hazard:null,speed:sp,
        x:fa.x+(fb.x-fa.x)*alpha,y:fa.y+(fb.y-fa.y)*alpha,vx:fb.vx,vy:fb.vy,angle:fb.angle,
        wobble:(Number(fb.angle)||0)+performance.now()/700,finPhase:performance.now()/180,life:0,coin:false,
        _chasing:false,_chaseTime:0,_attackCooldown:0});
    }
    g.entities=out;
    if(me){
      g.score=Number(me.score)||0;g.growthScore=Number(me.growthScore??me.score)||0;g.lives=Number(me.lives)||0;
      if(g.player){
        const dx=me.x-g.player.x,dy=me.y-g.player.y,er=Math.hypot(dx,dy);
        if(er>140){g.player.x=me.x;g.player.y=me.y}else{g.player.x+=dx*.22;g.player.y+=dy*.22}
        g.player.vx=me.vx;g.player.vy=me.vy;g.player.angle=me.angle;
      }
    }
  }catch{}
};
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(u.pathname==='/health'){
      ...
      return res.end(JSON.stringify({ok:true,server:'live-authoritative-v3',worlds:['PVP','PVE','EQUIPO'],fishPerWorld:FISH_N,tickHz:TICK_HZ,snapshotHz:SNAP_HZ,
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    if(m.type==='join'){
      if(joined)return;
      joined=true;
      const [code,mode]=worldForMode(m.mode);world=worlds.get(code);
      ...
      if(!p){
        ...
        const pos=spawn(world);
        p={id:deviceId||makeId('player'),resumeId:resumeId||makeId('resume'),deviceId:deviceId||makeDeviceId(),ws:null,connected:false,disconnectedAt:0,
          name:safeName(m.name),team:teamFor(world,m.team),x:pos.x,y:pos.y,tx:pos.x,ty:pos.y,lastX:pos.x,lastY:pos.y,vx:0,vy:0,angle:0,
          score:0,radius:11,level:1,lives:1,alive:true,sprinting:false,lastInputAt:Date.now(),invulnerableUntil:Date.now()+1200};
        world.players.set(p.id,p);
      }
      p.ws=ws;p.connected=true;p.disconnectedAt=0;p.name=safeName(m.name||p.name);p.team=p.team||teamFor(world,m.team);p.mode=mode;
      player=p;
    ...
    else if(m.type==='consume') tryConsume(world,player,m.entityId);
    else if(m.type==='boss_defeated'){
      if(world.stage===1)world.stage=2;
      else if(world.stage===3)world.stage=4;
  ...
  ws.on('close',()=>{
    ...
    sendAll(world,{type:'player_left',id:player.id});
    broadcast(world);
  });
});
...
setInterval(()=>{
  ...
  for(const w of worlds.values()){w.tick++;movePlayers(w);updateFish(w,dt);combat(w);}
  if(now>=nextSnap){nextSnap=now+1000/SNAP_HZ;for(const w of worlds.values())broadcast(w);}
},1000/TICK_HZ);

server.listen(PORT,'0.0.0.0',()=>console.log(`NANY LIVE AUTHORITATIVE V3 ${PORT}`));
