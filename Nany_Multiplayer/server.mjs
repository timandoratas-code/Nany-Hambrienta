...
function snapshotFor(room,p,now){
  const entities=[];for(const e of room.entities.values())if(entityVisibleTo(p,e))entities.push({id:e.id,type:e.type,variant:e.variant,size:e.size,baseSize:e.baseSize,points:e.points,power:e.power,speed:e.speed,x:+e.x.toFixed(2),y:+e.y.toFixed(2),vx:+e.vx.toFixed(2),vy:+e.vy.toFixed(2),angle:+e.angle.toFixed(3),wobble:e.wobble,finPhase:e.finPhase,boss:!!e.boss,bossType:e.bossType,bossHits:e.bossHits||0,vulnerableTail:!!e.vulnerableTail,chasing:!!e.chasing});
