...
/* -------------------------------------------------------------------------
   MULTIPLAYER: COLISIÓN OFFLINE + COMPENSACIÓN DE LATENCIA
   -------------------------------------------------------------------------
   Offline ya funciona bien. El problema online es temporal: el jugador local
   y el servidor no están exactamente en el mismo instante. Cuando el cliente
   detecta el MISMO contacto que offline, manda un snapshot pequeño del contacto
   (posición del jugador + pez). El servidor lo valida contra su estado reciente
   con una tolerancia acotada y aplica la misma regla de tamaño offline.
   ------------------------------------------------------------------------- */
const directTryConsume=`function __validClientContact(p,f,c,reach){
  ...
  // El cliente solo puede compensar una distancia razonable de red, nunca
  // teletransportar una colisión desde otra zona del mapa.
  const playerTolerance=Math.max(260,radius(p.growthScore)*3.5);
  const fishTolerance=Math.max(220,(Number(f?.size)||0)*5.0);
  if(Math.hypot(px-p.x,py-p.y)>playerTolerance)return false;
  if(Math.hypot(fx-f.x,fy-f.y)>fishTolerance)return false;

  // La colisión reportada debe ser exactamente la misma regla que offline.
  return Math.hypot(px-fx,py-fy)<=reach+5;
}
function tryConsume(w,p,id,contact=null){
  ...
  if(dist(p,f)>reach&&!__validClientContact(p,f,contact,reach))return false;
...
function tryPredatorContact(w,p,id,contact=null){
  ...
  if(!f||!canFishEatPlayer(p,f))return false;
  const reach=playerHB(p.growthScore)+fishHB(f);
  if(dist(p,f)>reach&&!__validClientContact(p,f,contact,reach))return false;
...
/* Combat del servidor conserva exactamente las reglas geométricas offline.
   Los mensajes de contacto solo compensan el tiempo de red entre snapshots. */
const directCombat=`function combat(w){
  ...
  for(const p of w.players.values()){
    ...
    for(const f of [...w.fish.values()]){
      ...
      if(canFishEatPlayer(p,f)){killPlayer(w,p,'fish',null,f);break;}
    }
    if(!p.alive)continue;
    const h=w.lavaHazard;
    if(h&&dist(p,h)<=ph+fishHB(h)){killPlayer(w,p,'fish',null,h);continue;}
    const b=w.boss;
    if(b&&b.chasing&&b.chaseId===p.id&&b.chaseUntil>now&&dist(p,b)<=ph+b.size*.72){killPlayer(w,p,'boss',null,b);continue;}
  }
  ...
  for(let i=0;i<ps.length;i++)for(let j=i+1;j<ps.length;j++){
    ...
    if(a.growthScore>b.growthScore*1.04&&ar>br*1.02){
      a.score+=Math.max(10,Math.floor(b.score*.25));a.growthScore+=Math.max(10,Math.floor(b.growthScore*.25));killPlayer(w,b,'player',a);
    }else if(b.growthScore>a.growthScore*1.04&&br>ar*1.02){
      b.score+=Math.max(10,Math.floor(a.score*.25));b.growthScore+=Math.max(10,Math.floor(a.growthScore*.25));killPlayer(w,a,'player',b);
    }
  }
  maybeStartBoss(w);
}`;
...
// Exportamos la ruta de contacto de depredador al servidor generado.
...
/* Pruebas de la misma lógica que usará la partida. */
...
function fish({id,role='prey',size=4,x=111,type='plankton',points=4}){return{id,index:0,type,role,points,color:'#fff',size,speed:1,baseSpeed:1,x,y:100,vx:0,vy:0,angle:0,heading:0,turn:1,chaseId:null,chaseUntil:0,chaseStartedAt:0,cooldownUntil:0,light:false,hazard:null,immortal:false,gemFish:false,renderKey:type};}
{
  ...
  if(!TestRuntime.tryConsume(w,p,f.id)||w.fish.has(f.id))throw new Error('Collision self-test FAILED: direct eat');
}
{
  // El servidor ve al pez 80 px más adelante, pero el cliente reporta el
  // contacto que realmente mostró en pantalla. Debe aceptarse como lag válido.
...
}
{
  ...
  if(!TestRuntime.tryPredatorContact(w,p,f.id,c)||p.alive)throw new Error('Collision self-test FAILED: lag compensated predator');
}
console.log('NANY COLLISION SELF-TEST OK: direct + lag-compensated contacts');
...
serverSource=serverSource.replace("server:'live-authoritative-v14-death-flow'","server:'live-authoritative-v17-client-contact'");
...
/* -------------------------------------------------------------------------
   CLIENTE: USA EL CONTACTO VISUAL QUE OFFLINE YA CONSIDERA CORRECTO
   ------------------------------------------------------------------------- */
// Quitamos el retraso artificial de render de 110 ms.
...
// Ligera extrapolación desde el último snapshot para no volver a dibujar peces
// en el pasado mientras llega el siguiente paquete del servidor.
...
// Evitamos repetir el mismo contacto de depredador cada frame.
...
// Sustituimos SOLO el bloque multiplayer que anulaba la mitad de las colisiones.
// Ahora replica la decisión offline y manda el snapshot del contacto al servidor.
...
const newCollision="if(!e.sharedId||PENDING.has(String(e.sharedId)))continue;const fh=fhFn?.(e)||Math.max(2.5,e.size*.88),edible=e.gemFish||(!e.immortal&&e.ecologyRole==='prey'&&fh<ph*.96),predator=!e.gemFish&&(e.ecologyRole==='predator'||e.role==='predator'||e.role==='hunter')&&fh>=ph*1.04,contact={px:Number(p.x)||0,py:Number(p.y)||0,fx:Number(e.x)||0,fy:Number(e.y)||0,angle:Number(p.angle)||0,clientTime:Date.now()};if(edible&&dd<=ph+fh){PENDING.add(String(e.sharedId));sendDirect({type:'consume',entityId:e.serverId||e.sharedId,points:e.points||1,contact});continue}if(predator&&dd<=ph+fh){const key=String(e.serverId||e.sharedId),now=performance.now(),last=CONTACT_SENT.get(key)||0;if(now-last>260){CONTACT_SENT.set(key,now);sendDirect({type:'predator_contact',entityId:e.serverId||e.sharedId,contact});}}";
...
clientSource="window.__NANY_COLLISION_BUILD__='v17-client-contact';\n"+clientSource;
