...
const MAX_PLAYERS = 8, MAX_STEP = 11, FISH_N = 260;
...
function buildFish(room){
  ...
  for(let i=0;i<FISH_N;i++){
    ...
    room.fish.set(`fish-${i}`,{id:`fish-${i}`,type:t.key,role:t.role,power:t.power,points:t.points,color:t.color,size:t.size*(.88+r()*.24),speed,x:100+r()*(W-200),y:100+r()*(H-200),vx:Math.cos(a)*speed*.55,vy:Math.sin(a)*speed*.55,angle:a,heading:a,turn:.8+r()*1.7,phase:r()*Math.PI*2,chaseId:null,chaseUntil:0,cooldownUntil:0,hitUntil:0});
  }
}
function makeRoom(code,mode){
  const room={code,mode,seed:(Math.floor(Math.random()*0xffffffff)>>>0)||1,epoch:Date.now(),players:new Map(),fish:new Map(),anchor:{x:W/2,y:H/2}};
...
function welcome(room,p){return{type:'welcome',id:p.id,room:room.code,mode:room.mode,team:p.team,teamName:p.team==='A'?'Azul':p.team==='B'?'Rojo':null,worldSeed:room.seed,worldEpoch:room.epoch,worldStage:0,boss:null,players:[...room.players.values()].map(pubPlayer),you:pubPlayer(p),player:pubPlayer(p),entities:[...room.fish.values()].map(pubFish)};}
  ...
  for(const p of room.players.values()){
    ...
    for(const f of room.fish.values()){
      ...
      if(f.power<pp&&f.size<p.radius*.98){
        ...
        p.x=clamp(room.anchor.x+(Math.random()-.5)*220,100,W-100);p.y=clamp(room.anchor.y+(Math.random()-.5)*220,100,H-100);
...
function inject(html){
  const bridge=`<script>(()=>{
    ...
    function WrappedWebSocket(url,protocols){
      ...
      ws.addEventListener('message',ev=>{try{const m=JSON.parse(ev.data);if(m&&(m.type==='welcome'||m.type==='snapshot'))window[STATE]=m;}catch(_){}});
      return ws;
    }
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(u.pathname==='/health'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(JSON.stringify({ok:true,rooms:rooms.size,players:[...rooms.values()].reduce((n,r)=>n+r.players.size,0),fishPerRoom:FISH_N}));}
    if(u.pathname==='/'||u.pathname==='/index.html'){const html=inject(await fs.readFile(path.join(ROOT,'index.html'),'utf8'));res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'});return res.end(html);}
...
wss.on('connection',ws=>{
  ...
  ws.on('close',()=>{if(!p||!room)return;room.players.delete(p.id);if(!room.players.size)rooms.delete(room.code);else broadcast(room);});
});
let last=Date.now(),lastSnapshot=Date.now();
setInterval(()=>{const now=Date.now(),dt=Math.min(.05,(now-last)/1000);last=now;for(const room of rooms.values()){movePlayers(room);updateFish(room,dt);combat(room);}if(now-lastSnapshot>=1000/SNAP_HZ){lastSnapshot=now;for(const room of rooms.values())broadcast(room);}},1000/TICK_HZ);
server.listen(PORT,'0.0.0.0',()=>console.log(`NANY AUTHORITATIVE MULTIPLAYER ${PORT}`));
